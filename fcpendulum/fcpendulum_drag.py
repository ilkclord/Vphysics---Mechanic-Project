...
g1 = graph(title = "Foucault Pendulum with air drag force" , width=1000, height=800)
f1 =  gcurve(color=color.blue) #drag
f2 = gcurve(color= color.red ) # without drag
...
vfz = 0
while(t < 320) :
    # if changing the direction , recalculate the fix term
    ...
    oz = oz  + dvz * dt + vz * dt
    f2.plot(oxp,ozp)
    if(abs(vx) <= abs(getvx(t)) and abs(vz) <= abs(getvz(t))):
        f1.plot(ox , oz)
    px = vx
    pz = vz
    vfx = dvx
